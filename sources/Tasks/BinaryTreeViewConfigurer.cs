...
namespace VSU.Tasks
{
    public class BinaryTreeViewConfigurer : ITaskConfigure<Student>
    {
        private const int MaxDepth = 3;
        private readonly CachedState _state;
        private readonly TreeView _view;

        public BinaryTreeViewConfigurer(CachedState state, TreeView view)
        {
            _state = state;
            _view = view;
        }
        ...
        public void Configure(IEnumerable<Student> collection)
        {
            if (!_state.Cached)
            {
                ...
                var tree = new BinaryTree<TaskStudent>();
